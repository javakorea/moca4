...
public class EgovMenuCreateManageServiceImpl extends EgovAbstractServiceImpl implements EgovMenuCreateManageService {
	...
	public List<?> selectMenuByAuth(MenuCreatVO vo) throws Exception{
		return menuCreateManageDAO.selectMenuByAuth(vo);
	}
	...
	public void insertMenuCreatList(Map map) throws Exception {
		//String checkedAuthorForInsert, String checkedMenuNoForInsert
		MenuCreatVO menuCreatVO = null;
		int AuthorCnt = 0;
		...
		List menuList = (List)map.get("paramList");
		for (int i = 0; i < menuList.size(); i++) {
			Map row = (Map)menuList.get(i);
			Object chk = row.get("chk");
			Class cls = chk.getClass();
			String cName = cls.getName();
			System.out.println("cName--->"+cName);
			int iValue = 0;
			if("java.lang.String".equalsIgnoreCase(cName)) {
				String chkd =(String)row.get("chk");
				System.out.println("chkd1--->"+chkd+"<-");
				iValue = Integer.parseInt(chkd);
			}else {
				Double chkd =(Double)row.get("chk");
				System.out.println("chkd2--->"+chkd+"<-");
				iValue = (int)chkd.doubleValue();
			}

			if(iValue == 1) {
				MenuCreatVO menuCreatVO2 = new MenuCreatVO();
				menuCreatVO2.setAuthorCode(authorCode);
				Double menuNo =(Double)row.get("menuNo");
				menuCreatVO2.setMenuNo((int)menuNo.doubleValue());
				menuCreateManageDAO.insertMenuCreat(menuCreatVO2);
			}
		}
	}
