...
public class EgovFileTool {
		...
		for (int i = 0; i < fileArray.length; i++) {
			...
			if (fileArray[i].isDirectory()) {
				...
				if (Globals.OS_TYPE.equals("UNIX")) {
					String[] command = { EgovStringUtil.isNullToString(EgovProperties.getProperty(Globals.SHELL_FILE_PATH), "SHELL." + Globals.OS_TYPE + ".getDrctryByOwner"),
	...
	public static String getMountLc(String file) throws Exception {
		...
		if (srcFile.exists()) {
			...
			if (Globals.OS_TYPE.equals("UNIX")) {
				Process p = null;
				String[] command = { EgovStringUtil.isNullToString(EgovProperties.getProperty(Globals.SHELL_FILE_PATH), "SHELL." + Globals.OS_TYPE + ".getMountLc"), src, "/" };
				p = Runtime.getRuntime().exec(command);
				//p.waitFor();
	...
	public static String getAccess(String file) throws Exception {
		...
		try {
			...
			if (srcFile.exists()) {
				...
				Process p = null;
				if (Globals.OS_TYPE.equals("UNIX")) {
					String[] command = { EgovStringUtil.isNullToString(EgovProperties.getProperty(Globals.SHELL_FILE_PATH), "SHELL." + Globals.OS_TYPE + ".getDrctryAccess"), parentPath, fname };
					p = Runtime.getRuntime().exec(command);
					p.waitFor();
					//프로세스 에러시 종료
					if (p != null && p.exitValue() != 0) {
